...
- **Nothing to install** — the core faces and the `base`/`document` packages are compiled into the
  artifact, so the dependency alone renders an ordinary document: Latin, Greek, Cyrillic,
