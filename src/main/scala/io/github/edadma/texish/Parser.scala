...
class Parser( commands: List[Command], actives: List[Active],
  ...
  def parse( src: io.Source ): AST =
    parseStatements( Reader.fromSource(src) ) match {
  ...
  def parseStatic( r: CharReader ): (CharReader, AST) = {
    val (r1, s) = consumeCond( r,
      r => !r.eoi && !lookahead(r, csDelim) && !lookahead(r, beginDelim) && !lookahead(r, endDelim) &&
  ...
  def parseControlSequenceName( r: CharReader ) =
    if (r eoi)
      ...
      val (r1, s) =
        if (nameFirst( r.ch ))
          consumeCond( r,
            r => !r.eoi && nameRest(r.ch) && !(r.ch == '.' && (r.next.eoi || !nameRest(r.next.ch))) )
  ...
  def consumeCond( r: CharReader, cond: CharReader => Boolean, buf: StringBuilder = new StringBuilder ): (CharReader, String) =
    if (cond( r )) {
      buf += r.ch
  ...
  def consume( r: CharReader, set: Char => Boolean, buf: StringBuilder = new StringBuilder ): (CharReader, String) = consumeCond( r, r => !r.eoi && set(r.ch), buf )
  ...
  def parseString( r: CharReader ) = consumeCond( r, r => !r.eoi && !r.ch.isWhitespace && !lookahead(r, csDelim) && !lookahead(r, beginDelim) && !lookahead(r, endDelim) )
...
//      case 0 => problem( pos, "illegal variable reference" )
  ...
  def parseRegularArguments( r: CharReader, n: Int, buf: ListBuffer[AST] = new ListBuffer[AST] ): (CharReader, List[AST]) = {
    if (n == 0)
      (r, buf toList)
    else {
  ...
  def parseExpressionArguments( r: CharReader, n: Int, buf: ListBuffer[AST] = new ListBuffer[AST] ): (CharReader, List[AST]) = {
    if (n == 0)
      (r, buf toList)
    else {
