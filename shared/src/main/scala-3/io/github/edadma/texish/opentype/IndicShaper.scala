...
/** Shapes Indic text by turning a run of codepoints into the glyphs to draw. An Indic script is written left
  ...
  *      post-base forms;
  *   4. reorder the vowel signs — the pre-base sign to the front of the cluster, or in a script that keeps its
  *      viramas visible to just before the base (it is typed after its consonant but drawn before it), and, in
  *      a script that subjoins its joined consonants, the sign drawn on the base back across them to the
  *      base's side — and append the reph mark after the base;
...
object IndicShaper:
  ...
  private val scripts: Seq[IndicScript] = Seq(Devanagari, Bengali, Gurmukhi, Telugu, Tamil)
...
final class IndicShaper(val script: IndicScript, gsub: Gsub):
  ...
  private def shapeCluster(clusterCps: Array[Int], cmap: Int => Int): Array[Int] =
    ...
    glyphs = script.reorderPreBaseMatra(coreCps, glyphs, preBaseGlyph, cmap(script.halant))
