...
class MarkTests extends AnyFreeSpec with Matchers:
  ...
  "interline glue is inserted across a mark, measured from the last box" in {
    ...
    class Line(val ascent: Double, val descent: Double) extends io.github.edadma.typesetter.ContentBox:
      val width: Double    = 10
      val xAdvance: Double = 10
      def draw(t: io.github.edadma.typesetter.Typesetter, x: Double, y: Double): Unit = ()
